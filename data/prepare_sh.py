...
'''set your data path'''
parser.add_argument('--data_path', type=str, default='/media/ShanghaiTech',
...
'''for part A'''
if not os.path.exists(part_A_train.replace('images', 'gt_detr_map_crop')):
    os.makedirs(part_A_train.replace('images', 'gt_detr_map_crop'))

if not os.path.exists(part_A_test.replace('images', 'gt_detr_map_crop')):
    os.makedirs(part_A_test.replace('images', 'gt_detr_map_crop'))
...
'''for part B'''
if not os.path.exists(part_B_train.replace('images', 'gt_detr_map_crop')):
    os.makedirs(part_B_train.replace('images', 'gt_detr_map_crop'))

if not os.path.exists(part_B_test.replace('images', 'gt_detr_map_crop')):
    os.makedirs(part_B_test.replace('images', 'gt_detr_map_crop'))
    ...
    if root_path.split('/')[-3] == 'train_data':
        ...
        for i in range(0, m):
            for j in range(0, n):
                ...
                h5_path = save_path.replace('.jpg', '.h5').replace('images', 'gt_detr_map')
                if gt_count == 0:
        ...
        with h5py.File(img_path.replace('.jpg', '.h5').replace('images', 'gt_detr_map'), 'w') as hf:
            hf['gt_count'] = gt_count
            hf['kpoint'] = kpoint