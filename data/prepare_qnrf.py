...
parser.add_argument('--data_path', type=str, default='/media/UCF-QNRF_ECCV18',
                    help='the data path of UCF-QNRF_ECCV18')
...
save_train_gt_path = root + '/train_data/gt_detr_map/'
save_test_img_path = root + '/test_data/images/'
save_test_gt_path = root + '/test_data/gt_detr_map/'
...
if not os.path.exists(save_train_img_path.replace('images', 'gt_show_detr')):
    os.makedirs(save_train_img_path.replace('images', 'gt_show_detr'))
...
if not os.path.exists(save_test_img_path.replace('images', 'gt_show_detr')):
    os.makedirs(save_test_img_path.replace('images', 'gt_show_detr'))
...
for k in range(len(img_train)):
    ...
    # Img_data_pil = Image.open(img_train_path).convert('RGB')
    ...
    #Img_data_pil = Img_data_pil.resize((Img_data.shape[1], Img_data.shape[0]), Img_data.ANTIALIAS)
    rate = rate_1 * rate_2
    ...
    gt_show_path = new_img_path.replace('images', 'gt_show_detr')
