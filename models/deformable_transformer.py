...
#             print("(~memory_padding_mask).sum(axis=-1)==============================")
            print((~memory_padding_mask).sum(axis=-1))
#             print("==============================")
...
#             print("valid_token_nums = "+str(valid_token_nums))
        if self.rho:
...
#             print("sparse_token_nums = "+str(sparse_token_nums))
            backbone_topk = min(backbone_topk, backbone_output_memory.shape[1])

#             print("backbone_topk = "+str(backbone_topk))
...
#             print("backbone_mask_prediction = "+str(backbone_mask_prediction))
            backbone_topk_proposals = torch.topk(backbone_mask_prediction, backbone_topk, dim=1)[1]
            
#             print("backbone_topk_proposals = "+str(backbone_topk_proposals))
        else:
        ...
        if self.two_stage:
            ...
            enc_outputs_coord_offset = self.decoder.point_embed[self.decoder.num_layers](output_memory)
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, 
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if self.aux_heads and lid < self.num_layers - 1:
                ...
                output_offset = self.point_embed[lid](tgt)
