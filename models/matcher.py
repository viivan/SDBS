...
#         if(tgt_point.cuda().shape[-1]!=out_point.shape[-1]):
#             p = torch.zeros([int(str(tgt_point.cuda().shape[0])),1]).to(device)
#             x2 = torch.cat((tgt_point.cuda(),p),dim=1).to(device)
#             cost_point = torch.cdist(out_point, x2.cuda(), p=1)
            
#         print(out_point.shape)
#         print(out_point)
#         print('```````````````````````````````````````````````````````````')
#         print(tgt_point.shape)
#         print(tgt_point)
#         else:
#         cost_point = torch.cdist(out_point[...,:3], tgt_point.cuda(), p=1)  #
